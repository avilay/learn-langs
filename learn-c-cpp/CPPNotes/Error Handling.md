...
Update: Why only return an `int`, I can return anything I want. Here are two different calling patterns that I can think of now -

```c
Cookie* cookie = bake(x, y);
if (!cookie) {
  // check the errno
}

consume(cookies);
// check the errno for void returns.
```
